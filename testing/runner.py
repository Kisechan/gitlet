...
CARGO_COMMAND = "cargo run --quiet --bin gitlet --"
CARGO_BUILD_COMMAND = "cargo build --quiet"
TIMEOUT = 10
DEBUG = False
DEBUG_MSG = \
    """
...
def doCompile(target):
    out = ""
    try:
        full_cmnd = CARGO_BUILD_COMMAND
        out = check_output(full_cmnd, shell=True, universal_newlines=True,
                           stdin=DEVNULL, stderr=STDOUT, cwd=gitlet_dir.strip('"'))
        return "OK", out
    except CalledProcessError as excp:
        return ("cargo build exited with code {}".format(excp.args[0]),
                excp.output)
...
def doExecute(cmnd, dir, timeout):
    ...
    try:
        chdir(dir)
        full_cmnd = "{} {}".format(CARGO_COMMAND, cmnd)
        ...
        if DEBUG:
            ...
            if next_cmd == "n":
                ...
                print("Set breakpoints in your Rust code and attach debugger!")
        ...
        return ("cargo run exited with code {}".format(excp.args[0]),
                excp.output)
...
if __name__ == "__main__":
    ...
    superverbose = False
    src_dir = 'samples/src'
    gitlet_dir = dirname(abspath(getcwd()))
    output_tolerance = 0
    ...
    if not isdir(gitlet_dir):
        print("Error: gitlet directory not found at {}".format(gitlet_dir), file=sys.stderr)
        sys.exit(1)

    msg, output = doCompile(gitlet_dir)
    if msg != "OK":
        print(output)
